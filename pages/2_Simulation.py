...
with _fc_col1:
    _sim_conf_levels = st.multiselect(
        "Entry Confidence-Stufen:", list(range(1, 11)), default=list(range(1, 11)),
        key="sim_conf_levels",
    )
with _fc_col2:
    _sim_close_conf_levels = st.multiselect(
        "Close Confidence-Stufen:", list(range(1, 11)), default=list(range(1, 11)),
        key="sim_close_conf_levels",
    )
with _fc_col3:
...
if st.button("◈ SIMULATION STARTEN", type="primary", use_container_width=True):
    _payload = {
        ...
        "close_confidence_levels": _sim_close_conf_levels,
