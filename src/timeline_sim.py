        ...
        for minute_idx, current_ts in enumerate(timeline):
            ...
            for asset, candles in asset_candles.items():
                ...
                if asset in open_trades:
                    ...
                    # High/Low-based peak for trade analytics (DB)
                    ...
                    # Close-based peak for v5 model state (matches training env)
                    _entry = tr["entry_price"] + 1e-8
                    _close_pnl_pct = (c_close - tr["entry_price"]) / _entry * 100.0
                    if not buy:
                        _close_pnl_pct = -_close_pnl_pct
                    if _close_pnl_pct > tr.get("_peak_close_pct", 0.0):
                        tr["_peak_close_pct"] = _close_pnl_pct
                ...
                if ci >= self._vcfg.max_window:
                    ...
                    if has_pos:
                        ...
                        # v5 extras (close-based peak, matching training env)
